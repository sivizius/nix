...
static void import(EvalState & state, const PosIdx pos, Value & vPath, Value * vScope, Value & v)
{
    ...
    if (auto optStorePath = isValidDerivationInStore()) {
        ...
        attrs.alloc(state.symbols.drvPath).mkString(path, {"=" + path});
        attrs.alloc(state.symbols.name).mkString(drv.env["name"]);
        auto & outputsVal = attrs.alloc(state.symbols.outputs);
...
static void prim_typeOf(EvalState & state, const PosIdx pos, Value * * args, Value & v)
{
    ...
    switch (args[0]->type()) {
        ...
        case nAttrPath: t = "attrPath"; break;
        case nThunk: abort();
    }
    v.mkString(t);
}
...
static void prim_genericClosure(EvalState & state, const PosIdx pos, Value * * args, Value & v)
{
    ...
    /* Get the start set. */
    Bindings::iterator startSet = getAttr(state, state.symbols.startSet, args[0]->attrs, "in the attrset passed as argument to builtins.genericClosure");
    ...
    if (startSet->value->isEmptyList()) {
        v = *startSet->value;
        return;
    }

    /* Get the operator. */
    Bindings::iterator op = getAttr(state, state.symbols.operator_, args[0]->attrs, "in the attrset passed as argument to builtins.genericClosure");
    ...
    while (!workSet.empty()) {
        ...
        Bindings::iterator key = getAttr(state, state.symbols.key, e->attrs, "in one of the attrsets generated by (or initially passed to) builtins.genericClosure");
...
static void prim_tryEval(EvalState & state, const PosIdx pos, Value * * args, Value & v)
{
    ...
    try {
        state.forceValue(*args[0], pos);
        attrs.insert(state.symbols.value, args[0]);
        attrs.alloc("success").mkBool(true);
    } catch (AssertionError & e) {
        attrs.alloc(state.symbols.value).mkBool(false);
...
[[gnu::noinline]]
void cannot_append(EvalState & state, const PosIdx pos, const Value & left, const Value & right)
{
    auto err = state.error("cannot append %1% to %2%", showType(right), showType(left));
    if (pos) {
        err.atPos(pos).debugThrow<EvalError>();
    } else {
        err.debugThrow<EvalError>();
    }
}

/* Append the latter argument to the former, e.g. "Hello" + "World" or 23 + 42.
 * This is the default implementation of the (+) operator, because unlike
 * the builtin __add, it is used for the non-commutative string concatination.
 */
static void prim_append(EvalState & state, const PosIdx pos, Value * * args, Value & value)
{
    auto left = *args[0];
    auto right = *args[1];
    ...
    switch (left.type()) {
        case nInt: {
            switch (right.type()) {
                case nInt:
                    value.mkInt(left.integer + right.integer);
                    return;
                case nFloat:
                    value.mkFloat(left.integer + right.fpoint);
                    return;
                default:
                    cannot_append(state, pos, left, right);
                    return;
            }
        }
        case nFloat: {
            switch (right.type()) {
                case nInt:
                    value.mkFloat(left.fpoint + right.integer);
                    return;
                case nFloat:
                    value.mkFloat(left.fpoint + right.fpoint);
                    return;
                default:
                    cannot_append(state, pos, left, right);
                    return;
        ...
        case nPath: {
            ...
            bool copyToStore = left.type() == nString;
            auto s1 = state.coerceToString(
                pos, left, context,
                "while evaluating the first argument of the string concatination",
                false, copyToStore, true
            );
            auto s2 = state.coerceToString(
                pos, right, context,
                "while evaluating the second argument of the string concatination",
                false, copyToStore, false
            );
            ...
            if (left.type() == nPath) {
                if (context.empty()) {
                    ...
                    result += *s1;
                    result += *s2;
                    value.mkPath(canonPath(result));
                } else {
                    state
                    .error("cannot append a string that refers to a store path to %1%", showType(left))
                    .atPos(pos)
                    .debugThrow<EvalError>();
                }
            } else {
                value.mkString(*s1, *s2, context);
            }
            return;
        }
        default: {
...
static void prim_curPos(EvalState & state, const PosIdx pos, Value * * args, Value & v)
{
    state.mkPos(v, pos);
}

static RegisterPrimOp primop_curPos({
    .name = "__curPos",
    .args = {},
    .doc = R"(
      Evaluates to an attribute set with *file*, *line* and *column* of its
      invocation.
    )",
    .fun = prim_curPos,
});
...
static void prim_derivationStrict(EvalState & state, const PosIdx pos, Value * * args, Value & v)
{
    ...
    Bindings::iterator nameAttr = getAttr(state, state.symbols.name, attrs, "in the attrset passed as argument to builtins.derivationStrict");
...
drvName, Bindings * attrs, Value & v)
{
    ...
    auto attr = attrs->find(state.symbols.__structuredAttrs);
    if (attr != attrs->end() &&
    ...
    attr = attrs->find(state.symbols.__ignoreNulls);
    if (attr != attrs->end())
    ...
    for (auto & i : attrs->lexicographicOrder(state.symbols)) {
        if (i->name == state.symbols.__ignoreNulls) continue;
        ...
        try {
            ...
            if (i->name == state.symbols.__contentAddressed) {
                contentAddressed = state.forceBool(*i->value, noPos, context_below);
                if (contentAddressed)
                    settings.requireExperimentalFeature(Xp::CaDerivations);
            }

            else if (i->name == state.symbols.__impure) {
            ...
            else if (i->name == state.symbols.args) {
            ...
            /* All other attributes are passed to the builder through
               the environment. */
                ...
                if (jsonObject) {
                    ...
                    if      (i->name == state.symbols.builder)        drv.builder = std::move(s);
                    else if (i->name == state.symbols.system)         drv.platform = std::move(s);
                    else if (i->name == state.symbols.outputHash)     outputHash = std::move(s);
                    else if (i->name == state.symbols.outputHashAlgo) outputHashAlgo = std::move(s);
                    else if (i->name == state.symbols.outputHashMode) handleHashMode(s);
                    else if (i->name == state.symbols.outputs)        handleOutputs(tokenizeString<Strings>(s));
                }
    ...
    result.alloc(state.symbols.drvPath).mkString(drvPathS, {"=" + drvPathS});
...
static void prim_findFile(EvalState & state, const PosIdx pos, Value * * args, Value & v)
{
    ...
    for (auto v2 : args[0]->listItems()) {
        ...
        std::string prefix;
        Bindings::iterator i = v2->attrs->find(state.symbols.prefix);
        if (i != v2->attrs->end())
        ...
        i = getAttr(state, state.symbols.path, v2->attrs, "in an element of the __nixPath");
...
static RegisterPrimOp primop_findFile({
    .name = "__findFile",
    .args = { "searchPaths", "relativePath" },
    .fun = prim_findFile,
});

static void prim_resolvePath(EvalState & state, const PosIdx pos, Value * * args, Value & v)
{
    auto path = state.forceStringNoCtx(*args[0], pos, "while evaluating the second argument passed to operator resolvePath");

    v.mkPath(state.checkSourcePath(state.findFile(path)));
}
...
static void prim_readDir(EvalState & state, const PosIdx pos, Value * * args, Value & v)
{
    ...
    for (auto & ent : entries) {
        ...
        if (ent.type == DT_UNKNOWN) {
            ...
            if (!readFileType)
                readFileType = &state.getBuiltin(state.symbols.readFileType);
...
static void prim_hasAttr(EvalState & state, const PosIdx pos, Value * * args, Value & v)
{
    ...
    switch (args[1]->type()) {
        ...
        default: {
            state.unexpectedType(*args[1], nAttrs, pos, "while evaluating the second argument passed to builtins.hasAttr");
        }
    }
}
...
static void prim_listToAttrs(EvalState & state, const PosIdx pos, Value * * args, Value & v)
{
    ...
    for (auto v2 : args[0]->listItems()) {
        ...
        Bindings::iterator j = getAttr(state, state.symbols.name, v2->attrs, "in a {name=...; value=...;} pair");
        ...
        if (seen.insert(sym).second) {
            Bindings::iterator j2 = getAttr(state, state.symbols.value, v2->attrs, "in a {name=...; value=...;} pair");
...
void prim_update(EvalState & state, const PosIdx pos, Value * * args, Value & v)
{
    state.forceAttrs(*args[0], pos, "while evaluating the first argument of the update operator (//)");
    state.forceAttrs(*args[1], pos, "while evaluating the second argument of the update operator (//)");

    state.nrOpUpdates++;

    if (args[0]->attrs->empty()) {
        v = *args[0];
    } else if (args[1]->attrs->empty()) {
        v = *args[1];
    } else {
        auto attrs = state.buildBindings(args[0]->attrs->size() + args[1]->attrs->size());

        /* Merge the sets, preferring values from the second set.  Make
          sure to keep the resulting vector in sorted order. */
        Bindings::iterator left = args[0]->attrs->begin();
        Bindings::iterator right = args[1]->attrs->begin();

        while (left != args[0]->attrs->end() && right != args[1]->attrs->end()) {
        ...
        while (left != args[0]->attrs->end()) {
            attrs.insert(*left++);
        }
        while (right != args[1]->attrs->end()) {
            attrs.insert(*right++);
        }
...
static void prim_functionArgs(EvalState & state, const PosIdx pos, Value * * args, Value & v)
{
    ...
    for (auto & i : args[0]->lambda.fun->formals->formals)
        // FIXME: should optimise booleans (allocate only once)
...
static void prim_concat(EvalState & state, const PosIdx pos, Value * * args, Value & v)
{
    state.forceList(*args[0], pos, "while evaluation the first argument of the list concatination (++)");
    state.forceList(*args[1], pos, "while evaluation the second argument of the list concatination (++)");
    state.concatLists(v, 2, args, pos, "while evaluating one of the elements to concatenate");
}
...
static void prim_partition(EvalState & state, const PosIdx pos, Value * * args, Value & v)
{
    ...
    auto & vRight = attrs.alloc(state.symbols.right);
    auto rsize = right.size();
    state.mkList(vRight, rsize);
    if (rsize != 0)
        memcpy(vRight.listElems(), right.data(), sizeof(Value *) * rsize);

    auto & vWrong = attrs.alloc(state.symbols.wrong);
...
static void prim_equal(EvalState & state, const PosIdx pos, Value * * args, Value & v)
{
    v.mkBool(state.eqValues(*args[0], *args[1], pos, "while testing two values for equality"));
}

static void prim_not(EvalState & state, const PosIdx pos, Value * * args, Value & v)
{
   v.mkBool(!state.forceBool(*args[0], pos, "while evaluating the argument of the logical inversion/not operator (!)"));
}

static void prim_logicAnd(EvalState & state, const PosIdx pos, Value * * args, Value & v)
{
    v.mkBool(
        state.forceBool(*args[0], pos, "while evaluating the first argument of the logical conjugation/and operator (&&)")
        && state.forceBool(*args[1], pos, "while evaluating the second argument of the logical conjugation/and opterator (&&)")
    );
}

static void prim_logicOr(EvalState & state, const PosIdx pos, Value * * args, Value & v)
{
    v.mkBool(
        state.forceBool(*args[0], pos, "while evaluating the first argument of the logical disjunction/or (||)")
        || state.forceBool(*args[1], pos, "while evaluating the second argument of the logical disjunction/or (||)")
    );
}

static void prim_imply(EvalState & state, const PosIdx pos, Value * * args, Value & v)
{
    v.mkBool(
        !state.forceBool(*args[0], pos, "while evaluating the first argument of the material conditional/imply operator (->)")
        || state.forceBool(*args[1], pos, "while evaluating the second argument of the material conditional/imply operator (->)")
    );
}
...
static void prim_negate(EvalState & state, const PosIdx pos, Value * * args, Value & v)
{
    state.forceValue(*args[0], pos);
    if (args[0]->type() == nFloat)
        v.mkFloat(-state.forceFloat(*args[0], pos, "while evaluating the argument of the negation"));
    else
        v.mkInt(-state.forceInt(*args[0], pos, "while evaluating the argument of the negation"));
}
...
static void prim_parseDrvName(EvalState & state, const PosIdx pos, Value * * args, Value & v)
{
    ...
    DrvName parsed(name);
    auto attrs = state.buildBindings(2);
    attrs.alloc(state.symbols.name).mkString(parsed.name);
...
RegisterPrimOp::RegisterPrimOp(std::string_view name, size_t arity, PrimOpFun fun)
{
...
void EvalState::createBaseEnv(const SymbolTable & s)
{
    baseEnv.up = 0;
    ...
    v.mkAttrs(buildBindings(128).alreadySorted());
    ...
    if (RegisterPrimOp::primOps) {
        for (auto & primOp : *RegisterPrimOp::primOps) {
            auto isExperimental = settings.isExperimentalFeatureEnabled(*primOp.experimentalFeature);
            if (!primOp.experimentalFeature || isExperimental) {
                addPrimOp({
                    .fun = primOp.fun,
                    .arity = primOp.arity,
                    .name = primOp.name,
    ...
    /* Add default implementations of the operators. */
    auto ops = buildBindings(15);
    ops.allocOp(s.logicNot,    "!",  {"e"},       "Return the logical inverse of *e*.",                prim_not);
    ops.allocOp(s.negate,      "-",  {"e"},       "Return the additive inverse of *e*.",               prim_negate);
    ops.allocOp(s.equal,       "==", {"e1","e2"}, "Return true, if *e1* and *e2* are equal.",          prim_equal);
    ops.allocOp(s.lessThan,    "<",  {"e1","e2"}, "Return true, if *e1* precedes *e2*.",               prim_lessThan);
    ops.allocOp(s.logicAnd,    "&&", {"e1","e2"}, "Return the logical conjunction of *e1* and *e2*.",  prim_logicAnd);
    ops.allocOp(s.logicOr,     "||", {"e1","e2"}, "Return the logical disjunction of *e1* and *e2*.",  prim_logicOr);
    ops.allocOp(s.imply,       "->", {"e1","e2"}, "Return the material conditional of *e1* and *e2*.", prim_imply);
    ops.allocOp(s.update,      "//", {"e1","e2"}, "Return *e1* updated by *e2* (e.g. sets).",          prim_update);
    ops.allocOp(s.has,         "?",  {"e1","e2"}, "Return true, if *e2* is in *e1* (e.g. a set).",     prim_hasAttr);
    ops.allocOp(s.concat,      "++", {"e1","e2"}, "Return *e1* concated with *e2* (e.g. lists).",      prim_concat);
    ops.allocOp(s.add,         "+",  {"e1","e2"}, "Return them sum of *e1* and *e2*.",                 prim_append);
    ops.allocOp(s.subtract,    "-",  {"e1","e2"}, "Return the difference of *e1* and *e2*.",           prim_sub);
    ops.allocOp(s.multiply,    "*",  {"e1","e2"}, "Return the product of *e1* and *e2*.",              prim_mul);
    ops.allocOp(s.divide,      "d",  {"e1","e2"}, "Return the quotient of *e1* and *e2*.",             prim_div);
    ops.allocOp(s.resolvePath, "",   {"path"},    "Return path relative to *NIX_PATH*.",               prim_resolvePath);
    v.mkAttrs(ops);
    addConstant("__operators", v);
